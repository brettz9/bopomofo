...
export const medials = [
  ...
  ['ㄥ', 'eng', null],
  ['ㄦ', 'er', null],
  // Occurs alone and followed by the previous
  //  (excepting ê (ㄝ), ao (ㄠ), ou (ㄡ), er (ㄦ))
  ['ㄧ', 'i', 'yi'],
  // Occurs alone and followed by the previous
  //   (excepting ê (ㄝ), ao (ㄠ), ou (ㄡ), er (ㄦ))
  ['ㄨ', 'u', 'wu'],
  // Occurs alone and followed only by these among the previous:
  //   ê (ㄝ), an (ㄢ), en (ㄣ), eng (ㄥ)
  ['ㄩ', 'ü', 'yu'],
  // Is different from all of the above in not being represented
  //  independently in transcription (except if including in
...
export const finalsToPinyin = [
  ...
  ...finals.map((finalChars) => {
    ...
    //  irregularities in how the first character is transformed as well:
...
export function findPinyinForBopomofoChars (finalChars, component = true) {
  ...
  const result = finalsToPinyin.find(([
    bpmf
    /* , pinyin */
  ]) => {
...
export async function getRandomEnhancedSyllable () {
  ...
  return possibleBopomofoSyllablesEnhanced[
    getRandomInt(possibleBopomofoSyllables.length - 1)
  ];
}

export const finals_single_nontranscriptional = finalsToPinyin.slice(0, 1);
export const finals_single = finalsToPinyin.slice(1).filter(
  ([bpmf]) => bpmf.length === 1
);
